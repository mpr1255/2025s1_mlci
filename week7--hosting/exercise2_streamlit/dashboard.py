...
A simple dashboard to explore the SpeakGer parliamentary speech database
"""
...
# Database path - resolve it properly
# When running via streamlit, __file__ might not be set correctly
# So we need to be more explicit
import os
_current_dir = Path(__file__).parent if '__file__' in globals() else Path.cwd()
DB_PATH = _current_dir.parent / "data" / "speakger.db"

# Fallback: if that doesn't exist, try relative to current working directory
if not DB_PATH.exists():
    DB_PATH = Path("../data/speakger.db").resolve()
...
def get_connection():
    ...
    if not DB_PATH.exists():
        ...
        st.info(f"Tried: {DB_PATH.absolute()}")
        st.info(f"Current dir: {Path.cwd()}")
        st.info(f"Script location: {Path(__file__).parent if '__file__' in globals() else 'unknown'}")
        st.stop()
    # Use URI mode to handle potential WAL files properly
...
LIMIT 10
    ...
    if st.button("Get Random Speech", type="primary"):
